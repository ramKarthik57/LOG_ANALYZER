...
from sentinel.forensics import (
    ForensicNarrator,
    GraphForensics,
    build_all_attack_chains,
    detect_insider_threats,
    detect_log_tampering,
    link_sessions,
)
from sentinel.parser import parse_log_file
from sentinel.plots import show_all_plots
from sentinel.report import generate_csv_report, generate_html_report
...
class SidebarButton(tk.Frame):
    def __init__(self, parent, icon, label, command, **kw):
        ...
        self._ico = tk.Label(
            inner, text=icon, font=("Consolas", 13), bg=C["sidebar"], fg=C["dim"]
        )
        self._ico.pack(side="left", padx=(0, 10))
        self._lbl = tk.Label(
            inner, text=label, font=FN_BTN, bg=C["sidebar"], fg=C["muted"], anchor="w"
        )
...
class KPI(tk.Frame):
    def __init__(self, parent, title, color=None, **kw):
        ...
        tk.Label(
            self, text=title.upper(), font=("Consolas", 7), bg=C["card"], fg=C["muted"]
        ).pack(anchor="w")
        self._v = tk.Label(
            self,
            text="—",
...
class SentinelGUI:
    ...
    def __init__(self, root):
        ...
        self.log(
            "SYSTEM", "Platform initialized. Awaiting log file or simulation command."
        )
    ...
    def _build_content(self):
        ...
        tk.Label(
            h,
            text="AI-Powered Log Forensics  ·  Threat Intelligence  ·  Attack Chain Reconstruction",
            font=FN_XS,
            bg=C["panel"],
            fg=C["muted"],
        ...
        for txt in self._txts.values():
            txt.tag_configure(
                "H", foreground=C["accent"], font=("Consolas", 11, "bold")
            )
            txt.tag_configure(
                "S", foreground=C["accent"], font=("Consolas", 10, "bold")
            )
            ...
            txt.tag_configure(
                "CRIT", foreground=C["red"], font=("Consolas", 10, "bold")
            )
    ...
    def _build_status(self):
        ...
        self._dot = tk.Label(bar, text="●", font=FN_XS, bg=C["sidebar"], fg=C["green"])
        self._dot.pack(side="left", padx=(12, 4))
        self._stat = tk.Label(
            bar, text="Ready", font=FN_XS, bg=C["sidebar"], fg=C["muted"]
        )
        self._stat.pack(side="left")
        tk.Label(
            bar, text="SENTINEL v1.0", font=FN_XS, bg=C["sidebar"], fg=C["dim"]
    ...
    def _tip(self, tab, tooltip_text):
        ...
        t.tag_bind(
            tag_name, "<Enter>", lambda e, txt=tooltip_text: self._show_tooltip(e, txt)
        )
    ...
    def load_file(self):
        ...
        if p:
            ...
            self.log(
                "FILE_LOAD", f"Successfully loaded external evidence log: {p}", "OK"
            )
    ...
    def _simulate_worker(self):
        try:
            ...
            self.file = p
            self.root.after(
                0, lambda: self._file_lbl.configure(text=f"[SIM] {os.path.basename(p)}")
            )
            self.root.after(0, lambda: self._status("Simulation ready", C["green"]))
            self.root.after(
                0,
                lambda: self.log(
                    "SIMULATE", f"Simulation compiled successfully to: {p}", "OK"
                ),
            )
            ...
            self.root.after(
                0,
                lambda msg=err_msg: self.log(
                    "ERROR", f"Simulation failed: {msg}", "FAIL"
                ),
            )
    ...
    def run_analysis(self):
        ...
        self.log(
            "PIPELINE",
            f"Commencing analytical pipeline on target: {os.path.basename(self.file)}",
        )
    ...
    def _pipeline(self):
        try:
            ...
            self.root.after(
                0,
                lambda: self.log(
                    "PHASE 1", "Multi-format log parsing and normalization"
                ),
            )
            ...
            if df.empty:
                self.root.after(
                    0,
                    lambda: self.log(
                        "PHASE 1", "No valid events parsed from file.", "FAIL"
                    ),
                )
                self.root.after(
                    0, lambda: messagebox.showerror("Error", "No events parsed.")
                )
                return
            ...
            self.root.after(
                0,
                lambda: self.log(
                    "PHASE 2", f"Enriching {len(df)} events with Threat Intel/GeoIP"
                ),
            )
            ...
            self.root.after(
                0,
                lambda: self.log(
                    "PHASE 3", "Running adaptive threshold detection heuristics"
                ),
            )
            ...
            self.root.after(
                0,
                lambda: self.log(
                    "PHASE 4", "Executing ML models (Isolation Forest, DBSCAN, HMM)"
                ),
            )
            ai = AIEngine()
            ...
            self.root.after(
                0,
                lambda: self.log(
                    "PHASE 5", "Evaluating multi-factor risk scores per IP"
                ),
            )
            scorer = RiskScoringEngine()
            self.severity_scores = scorer.score_all_ips(
                df, self.ai_results.get("anomaly")
            )
            ...
            self.root.after(
                ...
                lambda: self.log(
                    "PHASE 6",
                    "Reconstructing MITRE attack chains and tracking sessions",
                ),
            )
            ...
            self.root.after(
                0, lambda: self._status("7/7  SOAR Active Defense ...", C["amber"])
            )
            self.root.after(
                0,
                lambda: self.log(
                    "PHASE 7", "Executing automated response orchestration"
                ),
            )
            ...
            self.root.after(
                ...
                lambda: self.log(
                    "SUCCESS",
                    "Pipeline execution complete. Rendering forensic V2 views.",
                    "OK",
                ),
            )
            ...
            self.root.after(
                ...
                lambda: self.log(
                    "SUCCESS",
                    "Pipeline execution complete. Rendering forensic views.",
                    "OK",
                ),
            )
            ...
            self.root.after(
                0,
                lambda: self.log(
                    "FATAL", f"Exception in pipeline: {error_msg}", "CRIT"
                ),
            )
    ...
    def _fill_tabs(self):
        ...
        bf_flagged = (
            bf[bf.Is_Bruteforce] if "Is_Bruteforce" in bf.columns else bf.head(0)
        )
        ...
        n_anom = (
            len(anom_df[anom_df.is_anomaly])
            if anom_df is not None and "is_anomaly" in anom_df.columns
            else 0
        )
        ...
        header(
            R,
            "SENTINEL  —  FORENSIC INTELLIGENCE REPORT",
            "High-level managerial and executive summary of the entire investigation.",
        )
        ...
        section(
            R,
            "EXECUTIVE SUMMARY",
            "Distills the final automated assessments into actionable executive intelligence.",
        )
        threat_level = (
            "CRITICAL"
            if n_crit > 0
            else "HIGH" if n_high > 0 else "MEDIUM" if n_med > 0 else "LOW"
        )
        ...
        self._w(
            R,
            f"  Analysis of {n_total} authentication events across {dur_h:.1f} hours reveals:\n\n",
            "V",
        )
        ...
        if n_crit > 0:
            ...
            self._w(
                R,
                f"{n_crit} IP(s) classified CRITICAL — active compromise detected\n",
                "CRIT",
            )
            finding_num += 1
        if n_suspects > 0:
            ...
            self._w(
                R,
                f"{n_suspects} brute-force source(s) identified via dynamic thresholding\n",
                "FAIL",
            )
            finding_num += 1
        if not self.compromise_df.empty:
            ...
            self._w(
                R,
                f"{len(self.compromise_df)} confirmed FAIL→SUCCESS breach pattern(s)\n",
                "CRIT",
            )
            finding_num += 1
        if n_anom > 0:
            ...
            self._w(
                R,
                f"{n_anom} statistically anomalous IP(s) flagged by ML isolation layer\n",
                "HIGH",
            )
            finding_num += 1
        if chains:
            ...
            self._w(
                R,
                f"{len(chains)} attack chain(s) compiled (max killchain depth: {max_progress*100:.0f}%)\n",
                "WARN",
            )
        ...
        if "Geo_Country" in df.columns:
            section(
                R,
                "FOREIGN ACTOR PRESENCE",
                "Correlates attack sourcing with geographical IP assignments",
            )
            geo_counts = df.drop_duplicates("IP_Address").Geo_Country.value_counts()
            self._w(
                R, f"  {'Country':<22} {'IPs':>6} {'Events':>8} {'Avg Risk':>10}\n", "K"
            )
        ...
        section(
            ...
            "Identifies rapid automated guessing mechanics against SSH/authentication portals.",
        )
        det = self.bruteforce_df
        threshold = (
            det.Threshold.iloc[0]
            if "Threshold" in det.columns and not det.empty
            else "N/A"
        )
        self._w(
            R,
            f"  Methodology : Adaptive Z-Score (auto-calibrated threshold: {threshold} failed attempts)\n\n",
            "K",
        )
        if bf_flagged.empty:
            ...
            self._w(
                R,
                f"  {'IP Address':<22} {'Attempts':>10} {'Threshold':>11} {'Confidence':>12}\n",
                "K",
            )
            ...
            for _, row in bf_flagged.iterrows():
                self._w(
                    R,
                    f"  {row.IP_Address:<22} {row.Failed_Count:>10} {str(row.get('Threshold',''))[:11]:>11} {row.Confidence:>11.1%}\n",
                    "FAIL",
                )
        ...
        header(
            T,
            "EVENT TIMELINE",
            "Chronological logging of structural system access vectors.",
        )
        self._w(
            T,
            f"  Records: {n_total}  |  Time Window: {dur_h:.1f}h  |  Sessions: {n_sessions}\n\n",
            "K",
        )
        has_session = "Session_ID" in df.columns
        self._w(
            T, f"  {'Timestamp':<24} {'IP Address':<18} {'User':<14} {'Event':<18}", "K"
        )
        if has_session:
            self._w(T, f" {'Session':<10}", "K")
        self._w(T, "\n  " + "─" * 78 + "\n", "DIM")
        ...
        for _, row in df.sort_values("Parsed_Time").iterrows():
            ...
            self._w(
                T,
                f"  {str(row.Timestamp):<24} {row.IP_Address:<18} {row.get('Username','?'):<14} ",
                "K",
            )
            self._w(T, f"{row.Event:<18}", tag)
            if has_session:
                self._w(T, f" {cur_session:<10}", "DIM")
            self._w(T, "\n")
        ...
        header(
            ...
            "Deep-dives into weighted ensemble severity logic governing risk classification.",
        )

        self._w(S, "  Ensemble Logic : Score = ∑ (w_i * f_i)\n", "K")
        self._w(
            S,
            "  Thresholding   : CRITICAL >= 40  |  HIGH >= 25  |  MEDIUM >= 12  |  LOW < 12\n\n",
            "K",
        )

        self._w(
            S,
            f"  {'Rank':<6} {'IP Address':<20}  {'Score':<8}  {'Level':<12} {'Visual Heatmap'}\n",
            "K",
        )
        ...
        section(
            S,
            "DETAILED DOSSIERS",
            "Forensic factor breakdown justifying individual IP severity scores.",
        )
        for rank, (ip, d) in enumerate(sorted_scores, 1):
            if d["score"] < 5.0 and rank > 15:
                continue  # only show top for dossiers
            ...
            if factors:
                self._w(
                    S,
                    f"    {'Factor Engine':<26} {'Raw':>6}  {'Wt.Pts':>7}   {'Risk Intensity'}\n",
                    "K",
                )
        ...
        header(
            ...
            "Unsupervised algorithms targeting hidden statistical anomalies and multi-actor campaigns.",
        )

        section(
            A,
            "ISOLATION FOREST",
            "Evaluates high-dimensional IP feature vectors against normal organizational behavior.",
        )
        if anom_df is not None and not anom_df.empty:
            anomalies = anom_df.sort_values("anomaly_score", ascending=False)
            self._w(
                A,
                f"  {'IP Address':<18} {'Score':>10} {'Status':<12} {'Visual Contamination'}\n",
                "K",
            )
        ...
        section(
            A,
            "DBSCAN CLUSTERING",
            "Density-based spatial clustering to associate disparate IPs into coordinated campaigns.",
        )
        cluster_df = self.ai_results.get("clusters")
        if cluster_df is not None and not cluster_df.empty:
            camps = (
                cluster_df[cluster_df.campaign_cluster >= 0]
                if "campaign_cluster" in cluster_df.columns
                else cluster_df.head(0)
            )
            if camps.empty:
                self._w(
                    A,
                    "  Result: No coordinated IP syndicates found (all lone actors).\n",
                    "OK",
                )
            else:
                for cid in sorted(camps.campaign_cluster.unique()):
                    ...
                    self._w(
                        A,
                        f"  [Campaign ID:{cid:<3}] Identified {len(grp)} cooperative nodes: ",
                        "WARN",
                    )
        ...
        section(
            A,
            "HIDDEN MARKOV MODEL",
            "Probabilistic Viterbi decoding of observed user behavioral streams over time.",
        )
        self._w(
            A, f"  {'Username':<16} {'Current State':<16} {'Risk Probability'}\n", "K"
        )
        self._w(A, f"  {'─'*16} {'─'*16} {'─'*24}\n", "DIM")
        for user, p in sorted(
            self.user_profiles.items(), key=lambda x: x[1]["risk"], reverse=True
        ):
            tag = (
                "CRIT"
                if p["current_state"] == "COMPROMISED"
                else "WARN" if p["current_state"] == "ELEVATED" else "OK"
            )
        ...
        header(
            ...
            "Cyber-forensic causality linking, session mapping, and cryptographic log verification.",
        )

        section(
            F,
            "ATTACK CHAIN DAGs",
            "Directed acyclic graphs organizing events into the formal cyber kill chain framework.",
        )
        if chains:
        ...
        section(
            F,
            "SHANNON ENTROPY TAMPER DETECT",
            "Calculates entropy fluctuations across chronological events to expose log scrubbing techniques.",
        )
        if self.tamper_result and self.tamper_result.get("tampered"):
            ...
            for r in self.tamper_result["tamper_regions"]:
                self._w(
                    F,
                    f"  Anomaly located bounding lines {r[0]} to {r[1]} (z={r[3]:.1f})\n",
                    "FAIL",
                )
        else:
        ...
        header(
            M,
            "MITRE ATT&CK PIPELINE COVERAGE",
            "Direct translation of observed telemetry into standard MITRE operational taxonomies.",
        )

        if "MITRE_Technique" in df.columns:
            section(
                M,
                "TACTIC PHASE INTENSITY",
                "Relative saturation analysis indicating attacker objectives.",
            )
            ...
            for tactic, cnt in tactics.items():
                ...
                pct = cnt / n_total * 100
                tag = (
                    "FAIL" if "Privilege" in tactic or "Execution" in tactic else "WARN"
                )
            ...
            section(
                M,
                "OPERATIONAL TECHNIQUE DICTIONARY",
                "Isolated unique techniques mapped during forensic aggregation.",
            )
            mitre = df.MITRE_Technique.value_counts()
            self._w(
                M,
                f"  {'Signature':<14} {'Formal Designation':<30} {'Occurrences':>11}\n",
                "K",
            )
        ...
        header(
            AD,
            "SOAR - AUTONOMOUS RESPONSE LOG",
            "Live tracking of automated countermeasures executed by the SENTINEL active defense layer.",
        )
        soar_history = self.soar_engine.get_soar_logs()
        if not soar_history:
            self._w(
                AD, "  No automated defensive actions required at this time.\n", "OK"
            )
        else:
            self._w(
                AD,
                f"  {'Timestamp':<20} {'Action':<18} {'Target':<20} {'Status':<12}\n",
                "K",
            )
            ...
            for entry in soar_history:
                ...
                self._w(
                    AD,
                    f"  {entry['time']:<20} {entry['action']:<18} {entry['target']:<20} ",
                    "V",
                )
        ...
        header(
            AN,
            "AGENTIC FORENSIC NARRATIVE",
            "Agentic AI synthesis of technical evidence into natural language investigative reports.",
        )
        ...
        for ip, d in sorted_scores:
            ...
            ip_chain = next(
                (c for c in chains if c["ip"] == ip),
                {"ip": ip, "phases_reached": [], "kill_chain_progress": 0.0},
            )
    ...
    def open_config(self):
        ...
        tk.Label(w, text="SETTINGS", font=FN_H, bg=C["bg"], fg=C["accent"]).pack(
            pady=(18, 12)
        )
        tk.Button(w, text="CLOSE", font=FN_BTN, command=w.destroy, bg=C["accent"]).pack(
            pady=14
        )
