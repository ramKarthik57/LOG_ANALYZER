...
def _severity_badge(level: str) -> str:
    ...
    return (
        f'<span style="background:{c};color:#0a0e1a;padding:2px 10px;'
...
"""
    ...
    sorted_scores = sorted(
        severity_scores.items(), key=lambda x: x[1]["score"], reverse=True
    )
    for ip, data in sorted_scores:
        ...
        html += (
            f'<tr><td>{ip}</td><td>{data["score"]}</td>'
    ...
    if bruteforce_df is not None and not bruteforce_df.empty:
        bf = (
            bruteforce_df[bruteforce_df.Is_Bruteforce]
            if "Is_Bruteforce" in bruteforce_df.columns
            else bruteforce_df
        )
        if not bf.empty:
            ...
            for _, row in bf.iterrows():
                ...
                html += (
                    f"<tr><td>{row.IP_Address}</td><td>{row.Failed_Count}</td>"
    ...
    if attack_chains:
        for chain in attack_chains[:5]:
            ...
            for node in chain.get("nodes", [])[:10]:
                color = (
                    "#f59e0b"
                    if node["phase"] == "RECONNAISSANCE"
                    else (
                        "#ff6b35"
                        if node["phase"] == "INITIAL_ACCESS"
                        else (
                            "#ff2d55"
                            if node["phase"] == "PRIVILEGE_ESCALATION"
                            else "#00d4ff"
                        )
                    )
                )
        ...
        html += (
            '<div class="section">No significant attack chains reconstructed.</div>\n'
        )
    ...
    for ip, data in sorted_scores[:5]:
        ...
        for name, (raw, weighted, desc) in factors.items():
            ...
            html += (
                f'<div style="margin:3px 0"><span style="color:#64748b;font-size:11px">'
                f"{name}</span><br>"
...
def generate_csv_report(
    df: pd.DataFrame, severity_scores: dict, log_file: str, output_path: str = None
) -> str:
