...
class RiskScoringEngine:
    ...
    def score_ip(
        ...
        night = len(
            ip_df[(ip_df.Event == "SUCCESSFUL_LOGIN") & (ip_df.Parsed_Time.dt.hour < 5)]
        )
        ...
        if total > 1:
            time_span = (
                ip_df.Parsed_Time.max() - ip_df.Parsed_Time.min()
            ).total_seconds()
            ...
            return {
                ...
                "factors": {
                    "DECEPTION TRIGGER": (
                        100.0,
                        100.0,
                        "Interacted with Honey-Token (Zero False Positive)",
                    )
                },
            }
        ...
        total_score = sum(
            raw * self.WEIGHTS[wk]
            for (raw, _, _), wk in zip(factors.values(), self.WEIGHTS.keys())
        )
        ...
        level = (
            ...
            if total_score >= self.THRESHOLDS["CRITICAL"]
                ...
                else "MEDIUM" if total_score >= self.THRESHOLDS["MEDIUM"] else "LOW"
            )
        )
    ...
    def score_all_ips(self, df: pd.DataFrame, anomaly_df: pd.DataFrame = None) -> dict:
        """
