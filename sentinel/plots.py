...
def plot_attack_heatmap(df):
    ...
    pivot = (
        failed.pivot_table(index="dow", columns="hour", values="Event", aggfunc="count")
        .reindex(days)
        .fillna(0)
    )
...
def plot_fail_success(df):
    ...
    if mf > 3 and ms > 0:
        ax.axvspan(mf * 0.35, mf * 1.1, ymin=0.08, alpha=0.06, color=RED)
        ax.text(
            mf * 0.65,
            ms * 0.85,
            "COMPROMISE\nZONE",
    ...
    ax.set_title(
        "Failed vs Success Correlation  —  Compromise Map",
        color=CYAN,
        fontsize=12,
        fontweight="bold",
        pad=12,
...
def plot_login_frequency(df):
    ...
    ax.fill_between(freq.index, freq.values, alpha=0.2, color=CYAN)
    ...
    ax.set_title(
        "Login Frequency  —  Events per Minute",
        color=CYAN,
        fontsize=12,
        fontweight="bold",
        pad=12,
...
def plot_mitre_coverage(df):
    ...
    fig, ax = plt.subplots(figsize=(10, max(3.5, len(counts) * 0.55)))
    _style(fig, ax)

    colors = [RED if c > counts.median() else CYAN for c in counts.values]
    ax.barh(counts.index, counts.values, color=colors, edgecolor=BG, height=0.55)

    for i, (tech, cnt) in enumerate(counts.items()):
        ax.text(cnt + 0.5, i, str(cnt), va="center", color=WHITE, fontsize=9)
...
# SHOW ALL
# ═════════════════════════════════════════════════════════════
def show_all_plots(df, severity_scores=None, cluster_df=None, attack_chains=None):
